...
void Rectangle::MoveTo(Point newPosition) {
  auto dx = BottomRight.X - TopLeft.X;
  auto dy = BottomRight.Y - TopLeft.Y;

  TopLeft = Point(newPosition.X - dx, newPosition.Y + dy, newPosition.Z);
  BottomRight = Point(newPosition.X + dx, newPosition.Y - dy, newPosition.Z);
}

void Rectangle::MoveBy(Point dPosition) {
  TopLeft = PointOperations::Add(TopLeft, dPosition);
  BottomRight = PointOperations::Add(BottomRight, dPosition);
}

void Circle::MoveTo(Point newPosition) {
  Center = newPosition;
}

void Circle::MoveBy(Point dPosition) {
  Center = PointOperations::Add(Center, dPosition);
}
