...
namespace aabb
{
  ...
  public:
     ...
     */
    AABB(const std::vector<float32> &, const std::vector<float32> &);

    /// Compute the surface area of the box.
    float32 computeSurfaceArea() const;
     ...
     */
    std::vector<float32> computeCentre();
    ...
    std::vector<float32> lowerBound;

    /// Upper bound of AABB in each dimension.
    std::vector<float32> upperBound;

    /// The position of the AABB centre.
    std::vector<float32> centre;
  ...
  public:
     ...
     */
    Tree(unsigned int dimension_ = 3, float32 skinThickness_ = float32(0.05),
    ...
    /*! \param dimension_
        ...
        \param touchIsOverlap
            Does touching count as overlapping in query operations?
     */
    Tree(unsigned int, float32, const std::vector<bool> &, const std::vector<float32> &,
         unsigned int nParticles = 16, bool touchIsOverlap = true);
    ...
    /*! \param boxSize_
            The size of the simulation box in each dimension.
     */
    void setBoxSize(const std::vector<float32> &);
     ...
     */
    void insertParticle(unsigned int, std::vector<float32> &, float32);
    ...
    /*! \param index
        ...
        \param upperBound
            The upper bound in each dimension.
     */
    void insertParticle(unsigned int, std::vector<float32> &, std::vector<float32> &);
     ...
     */
    bool updateParticle(unsigned int, std::vector<float32> &, float32, bool alwaysReinsert = false);
    ...
    /*! \param particle
        ...
        \param alwaysReinsert
            Always reinsert the particle, even if it's within its old AABB (default: false)
     */
    bool updateParticle(unsigned int, std::vector<float32> &, std::vector<float32> &, bool alwaysReinsert = false);
     ...
     */
    float32 computeSurfaceAreaRatio() const;
  ...
  private:
    ...
    /* \param position
            The position vector.
     */
    void periodicBoundaries(std::vector<float32> &);
     ...
     */
    bool minimumImage(std::vector<float32> &, std::vector<float32> &);
  };
}
