...
namespace aabb
{
  ...
  AABB::AABB(const std::vector<float32> &lowerBound_, const std::vector<float32> &upperBound_) : lowerBound(lowerBound_), upperBound(upperBound_)
  {
  ...
  float32 AABB::computeSurfaceArea() const
  {
    ...
    for (unsigned int d1 = 0; d1 < lowerBound.size(); d1++)
    {
      ...
      for (unsigned int d2 = 0; d2 < lowerBound.size(); d2++)
      {
        ...
        float32 dx = upperBound[d2] - lowerBound[d2];
        product *= dx;
      }
  ...
  std::vector<float32> AABB::computeCentre()
  {
    std::vector<float32> position(lowerBound.size());

    for (unsigned int i = 0; i < position.size(); i++)
      position[i] = float32(0.5) * (lowerBound[i] + upperBound[i]);
  ...
  Tree::Tree(unsigned int dimension_,
             ...
             const std::vector<float32> &boxSize_,
             unsigned int nParticles,
                                     ...
                                     touchIsOverlap(touchIsOverlap_)
  {
    ...
    {
      posMinImage[i] = float32(0.5) * boxSize[i];
      negMinImage[i] = float32(-0.5) * boxSize[i];
  ...
  void Tree::setBoxSize(const std::vector<float32> &boxSize_)
  {
    boxSize = boxSize_;
  }
  ...
  void Tree::insertParticle(unsigned int particle, std::vector<float32> &position, float32 radius)
  {
    ...
    std::vector<float32> size(dimension);
  ...
  void Tree::insertParticle(unsigned int particle, std::vector<float32> &lowerBound, std::vector<float32> &upperBound)
  {
    // Make sure the particle doesn't already exist.
    if (particleMap.count(particle) != 0)
    {
  ...
  bool Tree::updateParticle(unsigned int particle, std::vector<float32> &position, float32 radius,
                            bool alwaysReinsert)
  {
    // Validate the dimensionality of the position vector.
    if (position.size() != dimension)
    ...
    // AABB bounds vectors.
    std::vector<float32> lowerBound(dimension);
    std::vector<float32> upperBound(dimension);
  ...
  bool Tree::updateParticle(unsigned int particle, std::vector<float32> &lowerBound,
                            std::vector<float32> &upperBound, bool alwaysReinsert)
  {
    // Validate the dimensionality of the bounds vectors.
    if ((lowerBound.size() != dimension) && (upperBound.size() != dimension))
    {
  ...
  std::vector<unsigned int> Tree::query(unsigned int particle, const AABB &aabb)
  {
    ...
    while (stack.size() > 0)
    {
      ...
      if (isPeriodic)
      {
        std::vector<float32> separation(dimension);
        std::vector<float32> shift(dimension);
        for (unsigned int i = 0; i < dimension; i++)
  ...
  void Tree::insertLeaf(unsigned int leaf)
  {
    ...
    while (!nodes[index].isLeaf())
    {
      ...
      float32 surfaceArea = nodes[index].aabb.getSurfaceArea();

      AABB combinedAABB;
      combinedAABB.merge(nodes[index].aabb, leafAABB);
      float32 combinedSurfaceArea = combinedAABB.getSurfaceArea();

      // Cost of creating a new parent for this node and the new leaf.
      float32 cost = float32(2.0) * combinedSurfaceArea;

      // Minimum cost of pushing the leaf further down the tree.
      float32 inheritanceCost = float32(2.0) * (combinedSurfaceArea - surfaceArea);
      ...
      else
      {
        AABB aabb;
        aabb.merge(leafAABB, nodes[left].aabb);
        float32 oldArea = nodes[left].aabb.getSurfaceArea();
        float32 newArea = aabb.getSurfaceArea();
      ...
      else
      {
        AABB aabb;
        aabb.merge(leafAABB, nodes[right].aabb);
        float32 oldArea = nodes[right].aabb.getSurfaceArea();
        float32 newArea = aabb.getSurfaceArea();
  ...
  float32 Tree::computeSurfaceAreaRatio() const
  {
    if (root == NULL_NODE)
      return float32(0.0);

    float32 rootArea = nodes[root].aabb.computeSurfaceArea();
    float32 totalArea = float32(0.0);
  ...
  void Tree::rebuild()
  {
    ...
    while (count > 1)
    {
      float32 minCost = std::numeric_limits<float32>::max();
      int iMin = -1, jMin = -1;
      ...
      for (unsigned int i = 0; i < count; i++)
      {
        ...
        for (unsigned int j = i + 1; j < count; j++)
        {
          ...
          float32 cost = aabb.getSurfaceArea();
  ...
  void Tree::periodicBoundaries(std::vector<float32> &position)
  {
    for (unsigned int i = 0; i < dimension; i++)
    {
      if (position[i] < float32(0))
      {
  ...
  bool Tree::minimumImage(std::vector<float32> &separation, std::vector<float32> &shift)
  {
    ...
    for (unsigned int i = 0; i < dimension; i++)
    {
      ...
      {
        separation[i] += float32((bool)(periodicity[i])) * boxSize[i];
        shift[i] = float32((bool)(periodicity[i])) * boxSize[i];
        isShifted = true;
      }
      else
      {
