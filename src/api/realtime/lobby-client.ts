...
import { mapLobby, mapLobbyList, mapSaveSlotList } from '@/api/realtime/lobby-mappers';
...
class LobbyClient {
  ...
  connect() {
    ...
    socket.onmessage = (event) => {
      ...
      switch (data.type) {
        case 'lobby_list':
          store.dispatch(lobbyActions.setLobbyList(mapLobbyList(data.lobbies)));
          return;
        case 'save_list':
          store.dispatch(lobbyActions.setSaveList(mapSaveSlotList(data.saves)));
          return;
        case 'lobby_state':
          ...
          store.dispatch(lobbyActions.setCurrentLobby(data.lobby ? mapLobby(data.lobby) : null));
          store.dispatch(lobbyActions.setErrorMessage(null));
